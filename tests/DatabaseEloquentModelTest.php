...
use Fluent\Orm\Contracts\Dispatcher;
// use Illuminate\Database\Connection;
// use Illuminate\Database\ConnectionResolverInterface;
// use Illuminate\Database\ConnectionResolverInterface as Resolver;
use Fluent\Orm\Builder;
use Fluent\Orm\Casts\ArrayObject;
use Fluent\Orm\Casts\AsArrayObject;
use Fluent\Orm\Casts\AsCollection;
use Fluent\Orm\Casts\AsStringable;
...
use Fluent\Orm\Relations\BelongsTo;
...
use Fluent\Orm\Support\Stringable;
...
class DatabaseEloquentModelTest extends TestCase
{
    ...
    public function testUpdateProcess()
    {
        ...
        $events->shouldReceive('dispatch')->once()->with('orm.updated: '.get_class($model), $model)->andReturn(true);
        $events->shouldReceive('dispatch')->once()->with('orm.saved: '.get_class($model), $model)->andReturn(true);
    ...
    public function testTimestampsAreReturnedAsObjectsOnCreate()
    {
        ...
        $model = new EloquentDateModelStub;
        Model::setConnectionResolver($resolver = m::mock(ConnectionInterface::class));
    ...
    {
        $model = $this->getMockBuilder('Fluent\Orm\Tests\EloquentDateModelStub')->onlyMethods(['getDateFormat'])->getMock();
    ...
    //     $events->shouldReceive('dispatch')->once()->with('orm.created: '.get_class($model), $model);
    //     $events->shouldReceive('dispatch')->once()->with('orm.saved: '.get_class($model), $model);
    ...
    {
        $model->setConnectionResolver($resolver = m::mock(ConnectionInterface::class));
        $resolver->shouldReceive('connection')->andReturn($connection = m::mock(BaseConnection::class));
